...
plot_models(modelo22)
plot_models(modelo22,rm.terms=c("Renda","faixa_etaria","MulherTRUE","Escolaridade",
                                "Raca_BrancaTRUE"))
...
anova_result <- aov(antiDemoc ~ perfilf, data = df)

# Resumo dos resultados
summary(anova_result)

# Realizar o teste de Kruskal-Wallis
kruskal_result <- kruskal.test(antiDemoc ~ perfilf, data = df)
...
tukey_result <- glht(anova_result, linfct = mcp(perfilf = "Tukey"))
...
tukey_comparisons <- TukeyHSD(anova_result, "perfilf")

# Converta os resultados em um data frame
tukey_df <- as.data.frame(tukey_comparisons$`perfilf`)
...
tukey_df$`p adj`
# Criação do gráfico de barras
ggplot(tukey_df, aes(x = reorder(row.names(tukey_df), diff), y = diff, ymin = lwr, ymax = upr, fill =`p adj` < 0.05)) +
...
ggplot(tukey_df, aes(x = reorder(row.names(tukey_df), diff), y = diff, ymin = lwr, ymax = upr, fill = `p adj` < 0.05)) +
  geom_errorbar(position = position_dodge(width = 0.9), width = 0.2) +
  geom_point(aes(x = reorder(row.names(tukey_df), diff), y = (lwr + upr) / 2), 
...
tukey_df$Comparação <- fct_recode(tukey_df$Comparação,
                                  "DM AntiBolson - Cat.Ref" = "DesengMoralAntiBolsonarista-cat_referencia",
                                  "DM AntiPetist - Cat.Ref" = "DesengMoralAntiPetista-cat_referencia",
                                  "DM AntiPetist - DM AntiBolson" = "DesengMoralAntiPetista-DesengMoralAntiBolsonarista",
                                  "Não DM - Cat.Ref" = "NaoDesengajado-cat_referencia",
                                  "Não DM - DM AntiBolson" = "NaoDesengajado-DesengMoralAntiBolsonarista",
                                  "Não DM - DM AntiPetist" = "NaoDesengajado-DesengMoralAntiPetista"
)
